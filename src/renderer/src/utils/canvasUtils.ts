...
export default async function getCroppedImg(
  ...
  // Resize to a reasonable size for avatars (e.g. 256x256) to save space
  const MAX_SIZE = 256
  ...
  if (scale < 1) {
    ...
    if (picaCtx) {
      picaCtx.drawImage(canvas, 0, 0, resizedCanvas.width, resizedCanvas.height)
      return resizedCanvas.toDataURL('image/jpeg', 0.85) // JPEG for smaller size
    }
  }
